...
int px,py,max,may,mbx,mby,mcx,mcy,mdx,mdy;
int apy,apx,sa,imr,pt,armor,cm,mal,mbl,mcl,mdl,amby,ambx,amay,amax,amdy,amdx;
int seed,rnd;
int i,j;
...
void cb()
{
	...
	if (a == 'k') {
		if (world[py][px] == '@') mal = 0,xp++,pt = 0;
		if ((world[py][px] == '&') && (xp > 0)) mbl = 0,xp+=3;
		if (world[py][px] == 'T') mdl = 0,xp++;
	}
	if ((a == 'n') && (xp >= 3)) {
		mal = 1;
		xp-=3;
...
void go_mb()
{
	...
	if (rw[mby][mbx] == '^') mby = rand() % 30,mbx = rand() % 50;
...
void go_md()
{
	...
	if (rnd == 3) mdx++;
	if (rnd == 4) mdx--;
	if (rw[mdy][mdx] == '#') mdy = amdy,mdx = amdx;
	if ((py == mdy) && (px == mdx)) {
		printf("\nT?");
		a = getch();
		if (a == 'y') {
...
void map()
{
	...
	if ((may > 29) || (max > 49) || (may < 0) || (max < 0)) may = 2,max = 2;
	if ((mby > 29) || (mbx > 49) || (mby < 0) || (mbx < 0)) mby = 2,mbx = 2;
	if ((mcy > 29) || (mcx > 49) || (mcy < 0) || (mcx < 0)) mcy = 2,mcx = 2;
	if ((mdy > 29) || (mdx > 49) || (mdy < 0) || (mdx < 0)) mdy = 2,mdx = 2;
	world[py][px] = syp;
	if (mal == 1) {
		world[may][max] = '@';
	}
...
int main()
{
	...
	printf("\nPlayer symbol:");
	scanf(" %c",&syp);
	py = 2,px = 2,may = rand() % 30,max = rand() % 50,mby = rand() % 30,mbx = rand() % 50,mdy = rand() % 30,mdx = rand() % 50,armor = 0,pt = 0;
	do
	{
		apy = py,apx = px;
		amby = mby,ambx = mbx;
