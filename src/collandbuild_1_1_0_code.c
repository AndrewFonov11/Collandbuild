...
void map()
{
	...
	if ((mdy > 19) || (mdx > 29) || (mdy < 0) || (mdx < 0)) mdy = 2,mdx = 2;
	world[py][px] = syp;
	if (mal == 1) {
		world[may][max] = '@';
	}
...
int main()
{
	...
	printf("\nPlayer symbol:");
	scanf(" %c",&syp);
	py = 2,px = 2,may = rand() % 20,max = rand() % 30,mby = rand() % 20,mbx = rand() % 30,mdy = rand() % 20,mdx = rand() % 30,armor = 0;
	do
	{
		apy = py,apx = px;
		amby = mby,ambx = mbx;
