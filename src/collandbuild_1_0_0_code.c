...
int hp,xp,stone,dirt,spines,gravel,flower,wood;
int px,py,max,may,mbx,mby,mcx,mcy;
int apy,apx,sa,imr,cm,mal,mbl,mcl,amby,ambx;
int seed,rnd;
int i,j;
...
void cb()
{
	...
	if (a == 'k') {
		if ((px == max) && (py == may)) mal = 0,xp++;
		if ((px == mbx) && (py == mby) && (xp > 0)) mbl = 0,xp+=3;
	}
	if ((a == 'n') && (xp > 3)) {
		mal = 1;
		xp-=3;
...
int main()
{
	...
	scanf(" %c",&a);
	if (a == 's') {
		hp = 5,xp = 0,stone = 0,dirt = 0,spines = 0,gravel = 0,flower = 0,wood = 0;
		mal = 1,mbl = 1,mcl = 0;
		grw();
	}
	if (a == 'e') {
		hp = 999,xp = 999,stone = 999,dirt = 999,spines = 999,gravel = 999,flower = 999,wood = 999;
		mal = 0,mbl = 0,mcl = 0;
		egrw();
	}
	printf("\nImmortal?(y/n)");
	...
	if (a == 'y') sa = 1;
	if (a == 'n') sa = 0;
	py = 2,px = 2,max = 10,may = 10,mbx = 15,mby = 15;
	do
	{
		apy = py,apx = px;
		amby = mby,ambx = mbx;
