...
int apy,apx,sa,imr,armor,cm,mal,mbl,mcl,mdl,amby,ambx,amay,amax,amdy,amdx;
int seed,rnd;
int i,j;
...
int main()
{
	...
	printf("\nPlayer symbol:");
	scanf(" %c",&syp);
	py = 2,px = 2,may = rand() % 30,max = rand() % 50,mby = rand() % 30,mbx = rand() % 50,mdy = rand() % 30,mdx = rand() % 50,armor = 0;
	do
	{
		apy = py,apx = px;
		amby = mby,ambx = mbx;
